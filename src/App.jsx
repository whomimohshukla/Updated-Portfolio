...
function TerminalCard() {
  ...
  const [history, setHistory] = useState([
    '✔ Ready in 0.22s — local: http://localhost:5173',
  ...
  const commands = {
    help: () => {
      append('Available: projects, skills, contact, about, github, linkedin, top, clear, help')
    },
    projects: () => {
      if (!scrollToSection('projects')) append('Could not find section: projects')
    },
    skills: () => {
      if (!scrollToSection('skills')) append('Could not find section: skills')
    },
    contact: () => {
      if (!scrollToSection('contact')) append('Could not find section: contact')
    },
    about: () => {
    ...
    github: () => {
      window.open('https://github.com/mimohshukla', '_blank', 'noopener,noreferrer')
    },
    linkedin: () => {
      window.open('https://www.linkedin.com/in/mimohshukla00/', '_blank', 'noopener,noreferrer')
    },
    clear: () => setHistory([]),
  }
  ...
  return (
    <div className="font-mono text-[12px] sm:text-[13px] leading-6">
      ...
      <div className="flex items-center text-[#00ef68] mt-1">
        ...
        />
        <span className="ml-1 h-4 w-[2px] bg-[#00ef68]" style={{ animation: 'caret 1s step-end infinite' }} />
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
        <path d="M3.5 16V8.8c0-1 .8-1.8 1.8-1.8.8 0 1.5.5 1.7 1.3L8.9 13c.2.6 1 .6 1.3 0l1.9-4.7c.3-.8 1-1.3 1.8-1.3 1 0 1.8.8 1.8 1.8V16" stroke="#00ef68" strokeWidth="2.2" strokeLinecap="round"/>
      </svg>
    </span>
  )
}
...
function Section({ id, title, children }) {
  return (
    <section
      id={id}
      className="py-20 px-6 sm:px-8 md:px-12 lg:px-20"
    >
      <Reveal>
        <h2 className="text-3xl md:text-4xl font-semibold tracking-tight mb-6 text-white">
          {title}
        </h2>
        <div className="text-gray-300 leading-relaxed max-w-3xl">{children}</div>
      </Reveal>
    </section>
  )
}

function Reveal({ children, as: Tag = 'div', className = '', delay = 0, y = 16 }) {
...
function ContributionsCalendar({ username }) {
      ...
      try {
        ...
        if (ghToken) {
          ...
          weeksArr = (cal.weeks || []).map(w => ({
            days: w.contributionDays.map(d => ({ date: d.date, count: d.contributionCount, level: 0 }))
          }))
          ...
          const res = await fetch(`https://github-contributions-api.jogruber.de/v4/${username}?y=${selectedYear}&ts=${Date.now()}`)
  ...
  const levelToClass = (lv) => {
    ...
    switch (lv) {
      case 1: return 'bg-[#005b32]'
      case 2: return 'bg-[#00a24e]'
      case 3: return 'bg-[#00c95e]'
      case 4: return 'bg-[#00ef68]'
          ...
          {weeks.length > 0 && (
            <span className="text-sm text-gray-400">{
              (typeof total === 'number' && !Number.isNaN(total)
                ? total
                : weeks.reduce((acc, w) => acc + w.days.reduce((a, d) => a + (d.count || 0), 0), 0)
              ).toLocaleString()
            } contributions</span>
          )}
            ...
            {[currentYear - 1, currentYear].map((y) => (
              <button
                ...
                className={`px-3 py-1 text-xs sm:text-sm transition-colors ${
                  selectedYear === y ? 'bg-[#00ef68] text-black' : 'bg-transparent text-gray-300 hover:bg-white/10'
                }`}
              >
                {y}
              </button>
            ...
            {weeks.map((w, wi) => (
              <div key={wi} className="contents">
                {w.days.map((d, di) => (
                  <div
                    key={`${wi}-${di}`}
                    className={`h-[10px] w-[10px] rounded-full ${levelToClass(d.level)}`}
...
function GithubStats({ username }) {
  const cardClass = "rounded-xl border border-white/10 bg-[#0f0f0f] p-3 hover:border-white/20 transition-colors"
  const imgClass = "w-full h-auto rounded-lg"

  return (
    <div className="max-w-6xl mx-auto grid gap-4 md:grid-cols-2">
      <Reveal as="a" className={cardClass} href={`https://github.com/${username}`} target="_blank" rel="noreferrer">
        <img
          className={imgClass}
          alt="GitHub stats"
      ...
      <div className="grid gap-4">
        <Reveal as="a" delay={80} className={cardClass} href={`https://github.com/${username}`} target="_blank" rel="noreferrer">
          <img
            className={imgClass}
            alt="GitHub Streak"
        ...
        <Reveal as="a" delay={140} className={cardClass} href={`https://github.com/${username}`} target="_blank" rel="noreferrer">
          <img
            className={imgClass}
      ...
      {repos.map((r) => (
        ...
        >
          <div className="flex items-center justify-between">
            <h4 className="text-white font-medium group-hover:underline">{r.name}</h4>
          ...
          <div className="mt-3 flex items-center gap-3 text-xs text-gray-400">
            {r.language && <span className="inline-flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-white/50" /> {r.language}
            </span>}
            {r.fork && <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10">fork</span>}
  ...
  const projects = [
    {
      title: 'Book My Bus',
      desc:
        'Full‑stack bus ticketing platform with searchable routes, seat selection, authentication, and admin tools.',
    ...
    },
    {
      ...
      desc:
        'Marketplace for clients and freelancers: post gigs, proposals, messaging, and secure payments flow.',
    ...
    },
    {
      ...
      desc:
        'Realtime chat application with rooms/DMs, presence, and typing indicators powered by websockets.',
    ...
    },
    {
      ...
      desc:
        'Lightweight weather dashboard that fetches live conditions by city with clean, responsive UI.',
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-black/60 bg-black/50">
        <nav className="max-w-6xl mx-auto flex items-center justify-between px-6 sm:px-8 md:px-12 lg:px-20 h-16">
          <a href="#home" className="group inline-flex items-center gap-0">
            <Logo size={26} />
            <span className="text-white font-medium tracking-tighter leading-none hidden sm:inline -ml-2 -mt-px">imoh</span>
          </a>
          <div className="hidden sm:flex items-center gap-8 text-sm">
            <a href="#about" className="relative text-gray-300 hover:text-white transition-colors after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#00ef68] after:transition-all hover:after:w-full">About</a>
            <a href="#skills" className="relative text-gray-300 hover:text-white transition-colors after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#00ef68] after:transition-all hover:after:w-full">Skills</a>
            <a href="#github" className="relative text-gray-300 hover:text-white transition-colors after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#00ef68] after:transition-all hover:after:w-full">GitHub</a>
            <a href="#projects" className="relative text-gray-300 hover:text-white transition-colors after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#00ef68] after:transition-all hover:after:w-full">Projects</a>
            <a href="#contact" className="relative text-gray-300 hover:text-white transition-colors after:absolute after:-bottom-1 after:left-0 after:h-[2px] after:w-0 after:bg-[#00ef68] after:transition-all hover:after:w-full">Contact</a>
          </div>
          <a
            href="#contact"
            ...
            <div className="flex items-center justify-center">
              <div className="relative w-full max-w-sm sm:max-w-md md:max-w-lg rounded-xl border border-white/10 bg-[#0f0f0f] backdrop-blur p-4 sm:p-5 shadow-2xl animate-[float_6s_ease-in-out_infinite]">
                <div className="flex items-center gap-1 mb-3 opacity-70">
                  ...
                  <span className="ml-auto text-[10px] text-gray-400">terminal</span>
                </div>
                <TerminalCard />
              </div>
            </div>
      ...
      <Section id="about" title="About Me">
        <p>
          I’m <strong className="text-white">Mimoh Shukla</strong>, a full stack developer. I enjoy shipping polished user
          experiences, designing clean APIs, and tuning performance. I work across the stack with React, Node.js,
          TypeScript, databases, and DevOps tooling to deliver production‑ready features quickly.
        </p>
      </Section>

      {/* Skills */}
      <section id="skills" className="py-16 px-6 sm:px-8 md:px-12 lg:px-20">
        <h2 className="text-3xl md:text-4xl font-semibold tracking-tight mb-6 text-white">Skills</h2>
        <SkillsGrid />
      </section>
      ...
      <section id="github" className="py-16 px-6 sm:px-8 md:px-12 lg:px-20">
        ...
        <div className="mt-10">
          <h3 className="text-xl font-semibold text-white mb-4">Public Repositories</h3>
      ...
      <section id="projects" className="py-20 px-6 sm:px-8 md:px-12 lg:px-20">
        <h2 className="text-3xl md:text-4xl font-semibold tracking-tight mb-8 text-white">Projects</h2>
      ...
      <Section id="contact" title="Get In Touch">
        <p>
          Have a project in mind or just want to say hi? Drop me a line and I’ll get back to you.
        </p>
        <form className="mt-6 grid gap-4 max-w-xl">
          <input className="w-full rounded-md bg-[#0f0f0f] border border-white/10 px-4 py-3 text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brownBlack" placeholder="Your name" />
          <input className="w-full rounded-md bg-[#0f0f0f] border border-white/10 px-4 py-3 text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brownBlack" type="email" placeholder="Email" />
          <textarea className="w-full rounded-md bg-[#0f0f0f] border border-white/10 px-4 py-3 text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-brownBlack" rows="4" placeholder="Message" />
          <button type="submit" className="inline-flex justify-center rounded-md bg-brownBlack px-5 py-3 text-sm font-medium text-white border border-white/10 hover:bg-brownBlack/90">
            Send Message
          </button>
        </form>
      </Section>
      ...
      <footer className="border-t border-white/10 py-10 bg-black">
        <div className="max-w-6xl mx-auto px-6 sm:px-8 md:px-12 lg:px-20 flex flex-col items-center gap-4">
          ...
          <p className="text-center text-sm text-gray-500">© {new Date().getFullYear()} mimohshukla — Full Stack Developer. Built with React, Vite, and Tailwind.</p>
          <p className="text-center text-sm">
            <a href="mailto:mimohshukl0001@gmail.com" className="text-gray-300 hover:text-white transition-colors">mimohshukl0001@gmail.com</a>
          </p>
        </div>
      </footer>
    </div>
