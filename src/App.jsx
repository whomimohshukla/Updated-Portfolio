	...
	const commands = {
		...
		skills: () => {
			if (!scrollToSection("skills"))
				append("Could not find section: skills");
		},
		contact: () => {
              ...
              user(login: $login) {
                contributionsCollection(from: $from, to: $to) {
                  contributionCalendar {
					...
					const counts = weeksArr.flatMap((w) =>
						w.days.map((d) => d.count)
					);
					...
					const q = (p) =>
						nonZero.length
							? nonZero[Math.floor((nonZero.length - 1) * p)]
							: 0;
					const t1 = q(0.25),
						t2 = q(0.5),
						t3 = q(0.75);
						...
						<span className='text-sm text-gray-400'>
							{(typeof total === "number" && !Number.isNaN(total)
								...
								: weeks.reduce(
										(acc, w) =>
											acc +
											w.days.reduce((a, d) => a + (d.count || 0), 0),
										0
								  )
							).toLocaleString()}{" "}
							contributions
	...
	return (
		<div className='min-h-screen bg-black'>
					...
					<div className='grid md:grid-cols-2 gap-8 md:gap-10 items-center'>
						...
						<div className='text-center md:text-left'>
							...
							<p className='mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base max-w-2xl mx-auto md:mx-0'>
								I design, build, and ship robust web apps end‑to‑end —
								from fast, accessible React UIs
							...
							<div className='mt-6 sm:mt-8 flex flex-wrap justify-center md:justify-start gap-3 sm:gap-4'>
								<a
									href='/whomimoshuklaFULLSTACK.pdf'
									target='_blank'
									rel='noreferrer'
									download
			...
			<Section id='about' title='About Me'>
				<p>
					I’m <strong className='text-white'>Mimoh Shukla</strong>, a full
					stack developer. I enjoy shipping polished user experiences,
					designing clean APIs, and tuning performance. I work across the
					stack with React, Node.js, TypeScript, databases, and DevOps
					tooling to deliver production‑ready features quickly.
				</p>
			</Section>
			...
			<footer className='border-t border-white/10 py-10 bg-black'>
				<div className='max-w-6xl mx-auto px-6 sm:px-8 md:px-12 lg:px-20 flex flex-col items-center gap-4'>
					...
					<p className='text-center text-sm text-gray-500'>
						© {new Date().getFullYear()} mimohshukla — Full Stack
						Developer. Built with React, Vite, and Tailwind.
					</p>
